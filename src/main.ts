...
class AlarmTerminal extends LitElement {
  ...
  private navButton(tab: Tab, label: string, icon: string) { return html`<button class=${this.tab === tab ? 'active' : ''} @click=${() => { this.tab = tab; }} aria-current=${this.tab === tab ? 'page' : 'false'}><span aria-hidden="true">${icon}</span> ${label}</button>`; }
  ...
  private renderAlarm(alarm: Alarm) { return html`<article class="alarm ${alarm.state}" role="button" tabindex="0" @click=${() => { this.selectedAlarm = alarm; }} @keydown=${(event: KeyboardEvent) => { if (event.key === 'Enter' || event.key === ' ') this.selectedAlarm = alarm; }}><div class="details"><h2>${alarm.title}</h2><p>${alarm.message}</p><time>${this.formatTime(alarm.time)}</time></div><div class="meta"><strong>${alarm.state === 'active' ? 'Active' : 'Acknowledged'}</strong><span class="source">${alarm.source}</span></div><div class="severity ${alarm.severity}"><b>${this.severityCode(alarm.severity)}</b><small>${alarm.state}</small></div></article>`; }
  ...
  private createTestAlarm = () => { const alarm: Alarm = { id: `local-${Date.now()}`, title: 'LOCAL.TEST_ALARM', message: 'Operator test event', source: 'This device', time: new Date().toISOString(), severity: 'warning', state: 'active' }; this.alarms = [alarm, ...this.alarms].slice(0, 100); this.persist(); this.status = 'Test alarm created'; this.tab = 'alarms'; };
  private acknowledge = (id: string) => { this.alarms = this.alarms.map((alarm) => alarm.id === id ? { ...alarm, state: 'acknowledged' } : alarm); this.persist(); this.status = 'Alarm acknowledged locally'; this.selectedAlarm = null; };
  private closeDialog = () => { this.selectedAlarm = null; };
  ...
  private onSoundChange = (event: Event) => { this.soundEnabled = (event.target as HTMLSelectElement).value === 'on'; localStorage.setItem('aog-sound', this.soundEnabled ? 'on' : 'off'); };
  private saveSettings = (event: SubmitEvent) => { event.preventDefault(); localStorage.setItem(ENDPOINT_STORAGE, this.endpoint.trim()); localStorage.setItem(VAPID_KEY_STORAGE, this.vapidKey.trim()); this.status = 'Settings saved locally'; };
