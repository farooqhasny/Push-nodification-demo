...
class AlarmTerminal extends LitElement {
  ...
  @state() private actionInFlight: 'subscribe' | 'unsubscribe' | null = null;
  ...
  static styles = css`
    ...
    * { box-sizing:border-box; } main { max-width:900px; min-height:100vh; margin:0 auto; padding-bottom:94px; background:#f5f7f8; }
    header { display:flex; align-items:center; gap:12px; padding:18px; color:#fff; background:#172024; } .mark { display:grid; place-items:center; width:38px; height:38px; color:#172024; background:#f5cc00; transform:rotate(45deg); font-weight:900; } .mark span { transform:rotate(-45deg); } .brand { display:grid; gap:3px; } .brand strong { font-size:21px; letter-spacing:.08em; } .brand small { color:#b9c5c8; font-size:10px; letter-spacing:.13em; } .live { display:grid; justify-items:end; gap:4px; margin-left:auto; font-size:10px; font-weight:800; letter-spacing:.05em; text-align:right; text-transform:uppercase; } .status-pill { padding:3px 6px; border-radius:4px; color:#172024; background:#d5dde0; } .status-pill.granted,.status-pill.active { color:#073b1a; background:#9ee2b5; } .status-pill.denied { color:#fff; background:#e93545; } .status-pill.default,.status-pill.inactive,.status-pill.unsupported { color:#172024; background:#f5cc00; }
  ...
  render() {
    ...
    return html`<main>
      <header><div class="mark"><span>A</span></div><div class="brand"><strong>AOG</strong><small>ALARM TERMINAL</small></div><div class="live"><span class="status-pill ${this.notificationPermission}">Permission: ${this.notificationPermission}</span><span class="status-pill ${this.pushEnabled ? 'active' : 'inactive'}">Subscription: ${this.pushEnabled ? 'active' : 'inactive'}</span></div></header>
  ...
  private get notificationPermission() { return 'Notification' in window ? Notification.permission : 'unsupported'; }
  ...
  private async enablePush() { this.actionInFlight = 'subscribe'; this.status = 'Requesting notification permission...'; this.refresh(); try { if (!('Notification' in window) || !('PushManager' in window)) throw new Error('Push is not supported by this browser.'); if (Notification.permission === 'denied') throw new Error('Notifications are blocked in browser settings.'); const permission = await Notification.requestPermission(); if (permission !== 'granted') throw new Error('Notification permission was not granted.'); this.status = 'Creating browser subscription...'; this.refresh(); const registration = await navigator.serviceWorker.ready; const existing = await registration.pushManager.getSubscription(); const subscription = existing || await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: this.decodeKey(this.vapidKey.trim()) }); const response = await fetch(this.endpoint.trim(), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'subscribe', subscription: subscription.toJSON() }) }); if (!response.ok) throw new Error(`Node-RED returned HTTP ${response.status}`); this.pushEnabled = true; this.status = 'Push subscription active'; } catch (error) { this.status = error instanceof TypeError ? 'Node-RED blocked this browser request. Enable CORS for this GitHub Pages origin.' : error instanceof Error ? error.message : 'Push setup failed'; } finally { this.actionInFlight = null; this.refresh(); } }
  private async disablePush() { this.actionInFlight = 'unsubscribe'; this.status = 'Removing push subscription...'; this.refresh(); try { const registration = await navigator.serviceWorker.ready; const subscription = await registration.pushManager.getSubscription(); if (subscription) { const response = await fetch(this.endpoint.trim(), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'unsubscribe', subscription: subscription.toJSON() }) }); if (!response.ok) throw new Error(`Node-RED returned HTTP ${response.status}`); await subscription.unsubscribe(); } this.pushEnabled = false; this.status = 'Push subscription removed'; } catch (error) { this.status = error instanceof TypeError ? 'Node-RED blocked this browser request. Enable CORS for this GitHub Pages origin.' : error instanceof Error ? error.message : 'Could not unsubscribe'; } finally { this.actionInFlight = null; this.refresh(); } }
